...
export default function NavbarDesktop({ activeTab, setActiveTab, onNavigateToHome, onNavigateToFeed, onNavigateToWardrobe, onNavigateToCanvas, onNavigateToSearch, onNavigateToBrand, onOpenAuth }) {
  return (
    <header className="sticky top-0 z-30 bg-[#FBFBFA]/95 backdrop-blur-md border-b border-[#EAE6DF]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <div className="flex items-center space-x-8">
          <button onClick={onNavigateToHome || onNavigateToFeed} className="font-serif-luxury text-2xl font-bold tracking-widest text-[#151816] hover:text-[#1A3C24] transition-colors">DRAPE</button>
        ...
        <div className="flex items-center space-x-4">
          ...
          <button onClick={onOpenAuth} className="p-2 text-[#56544E] hover:text-[#151816] hover:bg-[#EFECE6] rounded-full transition-colors relative" title="Notifications"><Bell className="w-4.5 h-4.5" /><span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-[#1A3C24] rounded-full" /></button>
          <button onClick={onOpenAuth} className="p-1 text-[#56544E] hover:text-[#151816] hover:bg-[#EFECE6] rounded-full transition-colors" title="Account Profile & Sign In"><div className="w-7 h-7 rounded-full bg-[#E5DFD5] border border-[#D5CEC2] flex items-center justify-center text-[#1A3C24]"><User className="w-4 h-4" /></div></button>
        </div>
      </div>
    </header>
  );
